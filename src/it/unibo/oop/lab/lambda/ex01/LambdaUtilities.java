...
public final class LambdaUtilities {
	...
	public static <T> List<Optional<T>> optFilter(final List<T> list, final Predicate<T> pre) {
		final List<Optional<T>> outputList = new ArrayList<>(list.size() * 2);
		list.forEach(e -> {
			final Optional<T> optionalElement = Optional.of(e);
			outputList.add(optionalElement.filter(pre));
		});
		return outputList;
	}
	...
	public static <R, T> Map<R, Set<T>> group(final List<T> list, final Function<T, R> op) {
		...
		list.forEach(e -> {
			output.merge(op.apply(e), new HashSet<>(Set.of(e)), (v1, v2) -> {
				v1.addAll(v2);
				return v1;
			});
		});
	...
	public static <K, V> Map<K, V> fill(final Map<K, Optional<V>> map, final Supplier<V> def) {
		...
		map.forEach((k, v) -> outputMap.put(k, v.orElseGet(def)));
		return outputMap;
	}
